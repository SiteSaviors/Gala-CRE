...
### 2026-09-22 — Client revisions Phase 0: asset and removal register

- Starting branch / HEAD: `main` at `e98d8d4`, clean; created `codex/september-client-updates` without changing, pulling, merging, or rewriting existing history.
- Source of truth checked: the current client email thread and supplied images/screenshots; `AGENTS.md`; Git status and recent local history; this ledger; `PROPERTY-DATA-REGISTER.md`; current Team, Contact, Properties, transaction, listing-page, video, navigation, and asset implementations; and relevant tests.
- Phase / checkpoint: client revisions Phase 0 — register three supplied portraits and identify every requested public removal, listing-order change, advisor relationship, portrait use, and video-layout change before altering site behavior.
- Gate: every supplied asset and removal target is identified without guessing; originals remain preserved; public behavior remains unchanged during registration.
- Implementation: created visually verified, web-ready WebP derivatives for Gaurang Gala, Goverdhan Vavilala, and Leigh Roach; recorded the authoritative eight-property order; mapped the Family Farm media-package exposure, six public document-button routes, all eight document-schema records, and the dormant legacy brochure path; mapped current and newly requested advisor assignments; recorded the current and requested video layouts; and documented the advisor-filter conflict that currently hides closed transactions.
- Files / routes: added `src/assets/team/gaurang-gala-2026.webp`, `src/assets/team/goverdhan-vavilala-2026.webp`, and `src/assets/team/leigh-roach-2026.webp`; updated `PROPERTY-DATA-REGISTER.md` and this ledger. No public route or component behavior changed.
- Verification: all three derivatives were inspected visually against their client-supplied originals; dimensions and bundle sizes verified as 1600×1067 / 64 KB, 1200×1800 / 172 KB, and 1066×1600 / 100 KB. The public-surface audit covered document/media URLs, portrait imports, property statuses/order, advisor IDs, transaction credits, advisor filtering, and inline-video behavior. Targeted Team, property, transaction, video, and page tests passed 47/47 across 5 files; `git diff --check` passed.
- Intended commit: `docs: register September client revision assets` after verification and scoped diff review.
...
### 2026-09-22 — Client revisions Phase 2: Team, Contact, and labeled Home navigation

- Starting branch / HEAD: `codex/september-client-updates` at `b3b98f4`, clean.
- Source of truth checked: the current Phase 2 request; `AGENTS.md`; `SEPTEMBER-CLIENT-REVISION-PLAN.md`; `PROPERTY-DATA-REGISTER.md`; the latest checkpoint in this ledger; Git branch, status, and recent local history; shared Team records; Team, Contact, SiteHeader, footer, responsive navigation, Contact-form context, CSS, and relevant tests and live routes.
- Phase / checkpoint: client revisions Phase 2 — finish the shared-data Team/Contact experience and add an explicit Home destination to desktop and mobile navigation.
- Gate: visitors can contact each agent through approved direct details, view the public work associated with that agent, and return home through an explicit labeled navigation link; current-location, keyboard, and responsive behavior remain intact.
- Implementation: retained the verified Team presentation completed with Phase 1; added a compact three-card Contact advisor directory sourced directly from `teamMembers`, with real portraits, titles, approved email/phone links, and portfolio links; added Home as the first labeled desktop and mobile navigation item while preserving the logo's home link; added section-aware `aria-current` states and visible active treatments; tightened desktop link spacing to accommodate the complete navigation; and updated `AGENTS.md` so future work starts from the active September plan rather than the historical rebuild checklist.
- Files / routes changed: `AGENTS.md`, `src/components/site/SiteHeader.tsx`, `src/pages/Contact.tsx`, `src/styles/gala.css`, `src/test/contact-page.test.tsx`, `src/test/capability-routes.test.tsx`, `PROPERTY-DATA-REGISTER.md`, `SEPTEMBER-CLIENT-REVISION-PLAN.md`, and this ledger; shared desktop/mobile navigation and `/contact`.
- Verification: targeted Contact/navigation/page suite passed 60/60 across 3 files; full Vitest suite passed 114/114 across 18 files; production build passed with only the existing browserslist-age and bundle-size notices; lint passed with 0 errors and the same 7 shared-UI Fast Refresh warnings. At 1280×720, all seven desktop links fit without overflow, Contact carried the active state, all three portraits loaded, and the advisor cards rendered as equal 383px columns. At 390×844, Home appeared first in the open menu, Contact remained visibly current, the 350px advisor cards stacked without horizontal overflow, and the page produced no console errors. Keyboard focus moved in order from Skip to Main Content to the logo and then Home. Inquiry-context tests remained green, and no form was submitted.
- Intended commit: `feat: add advisor contacts and home navigation` after scoped diff review.
- Remaining working tree: expected clean after commit.
- Blockers: none for this checkpoint. Direct email and telephone links use the client-approved information in the shared Team model; production form-delivery configuration remains a separate previously recorded launch input and was not exercised.
- Exact next action: Start Phase 3 with the catalog-order checkpoint by updating the eight structured property `sortOrder` values to the client-approved sequence, keeping the locked homepage Featured Listings order unchanged, adding catalog/advisor-order regression coverage, verifying `/properties` at desktop and mobile sizes, updating the active plan and checkpoint ledger, and creating one local commit.

### 2026-09-22 — Client revisions Phase 3: catalog order and private-material removal

- Starting branch / HEAD: `codex/september-client-updates` at `757a5b1219a9b33e038ce0f52ee7a62d876c2a34`, clean.
- Source of truth checked: the current Phase 3 request and corrected eight-property order; `AGENTS.md`; `SEPTEMBER-CLIENT-REVISION-PLAN.md`; `PROPERTY-DATA-REGISTER.md`; Git branch, status, and recent history; all structured property records; homepage featured-listing derivation; catalog/advisor filtering; commercial and fallback property renderers; Contact context handling; public assets; tests; and live local routes.
- Phase / checkpoint: client revisions Phase 3 — apply the approved listing order everywhere it feeds, retire all public property-document and media-package access, and preserve property/advisor context through a safe inquiry path.
- Gate: catalog, advisor-filtered results, and the explicitly authorized homepage active subset use the approved order; no internal or property-document material remains publicly accessible or in the production bundle; marketplace references remain secondary; and property inquiries preserve the primary assigned advisor without promising immediate document access.
- Implementation: changed the eight structured `sortOrder` values to Church, Family Farm, Lexington, Lackey, Yadkin, Bragg, North Main, and Chapel Hill; removed the property-document and brochure fields from the data model; deleted every document record, topic URL, document renderer, download action, and obsolete stylesheet; removed the Family Farm media-package URL; retained public Crexi/LoopNet references as secondary marketplace actions; added one shared advisor-aware property inquiry URL builder; replaced active listing close sections with **Request Property Information**; and made the Contact submission adapter explicitly merge URL-derived property and advisor context into the payload.
- Files / routes changed: `src/content/properties.ts`, `src/components/properties/CommercialListingPage.tsx`, `src/pages/PropertyDetail.tsx`, `src/pages/Contact.tsx`, `src/styles/gala.css`, `src/test/properties-model.test.ts`, `src/test/gala-pages.test.tsx`, `src/test/contact-page.test.tsx`, `e2e/public-route-matrix.spec.ts`, `PROPERTY-DATA-REGISTER.md`, `SEPTEMBER-CLIENT-REVISION-PLAN.md`, and this ledger; homepage Featured Listings, `/properties`, all advisor-filtered catalog views, all eight property routes, and property-context `/contact` journeys.
- Verification: targeted property/page/Contact suite passed 57/57; full Vitest suite passed 116/116 across 18 files; production build passed with the existing browserslist-age and bundle-size notices; lint passed with 0 errors and the same 7 shared-UI Fast Refresh warnings. The production-preview Playwright matrix passed 7/7 across 93 desktop/tablet/mobile route renders plus filters, map, gallery, video, inquiry-context, keyboard, and reduced-motion paths. Live local browser QA confirmed the exact catalog and homepage orders, 1440px Church Street and 390px Family Farm layouts without horizontal overflow, zero document/media-package surfaces, and advisor-aware inquiry URLs. No form was submitted. A filesystem and built-bundle scan found no PDF, Word, archive, or spreadsheet files and no forbidden download-center, media-package, brochure, or document-action strings.
- Intended commit: `fix: reorder listings and remove private materials` after scoped diff review.
- Remaining working tree: expected clean after the checkpoint commit.
- Blockers: none for Phase 3. North Main remains intentionally unassigned because no advisor attribution was supplied; its inquiry carries only the verified property context.
- Exact next action: Start Phase 4 by moving the existing conditional listing-video module into the opportunity section beside the narrative, placing the verified highlights beneath that narrative/video row, and verifying the shared layout first on Church Street at desktop and mobile sizes before applying it to Family Farm and Lexington.

### 2026-09-22 — Client revisions Phase 4: primary listing-video layout

- Starting branch / HEAD: `codex/september-client-updates` at `7d7b6e3fdf7ebc139fa364ae363f9516dd563f68`, clean.
- Source of truth checked: the current Phase 4 request and clarified conditional-module requirements; `AGENTS.md`; `SEPTEMBER-CLIENT-REVISION-PLAN.md`; `PROPERTY-DATA-REGISTER.md`; Git branch, status, and recent history; the shared commercial listing renderer; `PropertyVideo`; Church Street, Family Farm, and Lexington video records and assets; no-video property records; responsive styles; unit and browser tests; and live local routes.
- Phase / checkpoint: client revisions Phase 4 — move the existing conditional player into the primary opportunity module, place exactly three supporting points beneath the narrative/player row, and preserve a clean no-video presentation.
- Gate: on desktop, the narrative and a large listing video share the primary row and the three supporting points span beneath them; on mobile, the DOM and visual order is overview, video, then points; landscape and portrait videos retain posters, native controls, lazy loading, `playsInline`, and no autoplay; properties without video render no empty player surface.
- Implementation: made the opportunity section the single owner of conditional listing video; added accessible section labeling; moved player heading/context beneath the player; introduced a two-column desktop treatment with a full-width three-point row; stacked all three elements at mobile sizes; bounded portrait video to its natural 9:16 presentation; removed the duplicate former video branch and obsolete styles from Location & Media; and retained the established statement/highlight layout when `page.video` is absent.
- Files / routes changed: `src/components/properties/CommercialListingPage.tsx`, `src/styles/gala.css`, `src/test/gala-pages.test.tsx`, `e2e/public-route-matrix.spec.ts`, `PROPERTY-DATA-REGISTER.md`, `SEPTEMBER-CLIENT-REVISION-PLAN.md`, and this ledger; `/properties/611-703-church-street`, `/properties/5911-family-farm-road`, `/properties/1111-brown-street`, plus every non-video property route through the shared conditional renderer.
- Verification: targeted video/property tests passed 44/44; full Vitest passed 116/116 across 18 files; production build passed with only the existing browserslist-age and bundle-size notices; lint passed with 0 errors and the same 7 shared-UI Fast Refresh warnings. The production-preview Playwright matrix passed 7/7 across all 31 audited routes at 1440×900, 834×1112, and 390×844, including responsive layout order, three-point structure, player metadata, poster presence, no autoplay, video source loading, no duplicate location player, no empty no-video module, and horizontal-overflow checks. Live local browser QA passed for the Church landscape module, Lexington portrait module, and Lackey no-video path. No form was submitted.
- Intended commit: `refactor: elevate property listing videos` after scoped diff review.
- Remaining working tree: expected clean after the checkpoint commit.
- Blockers: none for Phase 4. Existing optimized videos and supplied poster assets were reused unchanged; no new facts, rights assumptions, or media were introduced.
- Exact next action: Start Phase 5 by auditing the Team contacts, agent portfolios, shared Chapel Hill attribution, Home navigation, approved property order, property inquiry context, absence of public document paths, and the three listing-video journeys; fix only verified defects and prepare the release/deployment handoff without deploying.

### 2026-09-22 — Client revisions Phase 5: relationship, privacy, and release certification

- Starting branch / HEAD: `codex/september-client-updates` at `51f86169b35c385378446adf9452899b3317f790`, clean.
- Source of truth checked: the current Phase 5 request and repeated Beth email; `AGENTS.md`; `SEPTEMBER-CLIENT-REVISION-PLAN.md`; `PROPERTY-DATA-REGISTER.md`; Git branch, status, and recent history; shared Team, property, transaction, navigation, Contact, form-routing, video, and privacy implementations; all unit and browser tests; source/public/build artifacts; local live routes; local branch relationship to `main` and `origin/main`; and the linked Vercel project's environment-variable inventory.
- Phase / checkpoint: client revisions Phase 5 — certify every requested agent relationship, privacy boundary, navigation/inquiry journey, responsive layout, video behavior, and release prerequisite, then leave a reproducible deployment handoff without promotion.
- Gate: no agent is incorrectly attributed; Gaurang, Leigh, and Goverdhan use their approved identities and direct contacts; each portfolio returns active and closed associated work; Chapel Hill carries shared credit without invented roles/economics; no confidential property document or retired URL remains public; and all requested journeys pass at desktop and mobile sizes.
- Implementation: added a dedicated September Playwright acceptance suite covering all three Team cards, portrait loading, email/telephone and action destinations, desktop/mobile Home navigation, exact advisor portfolios, the eight-property catalog order, every property inquiry context, shared Chapel Hill attribution boundaries, privacy removals, console errors, and horizontal overflow. Added `SEPTEMBER-RELEASE-HANDOFF.md` with the verified release scope, configuration blocker, remaining inputs, promotion sequence, production smoke tests, and local review command. No public route implementation required correction during the audit.
- Files / routes changed: `e2e/september-client-release.spec.ts`, `SEPTEMBER-RELEASE-HANDOFF.md`, `PROPERTY-DATA-REGISTER.md`, `SEPTEMBER-CLIENT-REVISION-PLAN.md`, and this ledger; browser certification covers `/team`, `/contact`, `/properties`, all three advisor-filtered catalogs, all eight property routes and inquiry contexts, all three video routes, and the existing 31-route property/service matrix.
- Verification: the focused September acceptance suite passed 3/3. The full Playwright suite passed 10/10, including 93 desktop/tablet/mobile property/service renders plus Team, portfolios, catalog, privacy, videos, inquiries, keyboard/focus, reduced motion, images, console, and overflow checks. Full Vitest passed 116/116 across 18 files. Production build passed with only the existing browserslist-age and bundle-size notices. Lint passed with 0 errors and the same 7 shared-UI Fast Refresh warnings. Static source/public/dist scans found no property-document files or forbidden retired URL/action/schema strings. Live local browser QA confirmed the three loaded Team portraits and the Goverdhan-to-Chapel Hill closed-record journey without overflow. No form was submitted.
- Intended commit: `test: certify September client release` after scoped diff review.
- Remaining working tree: expected clean after the checkpoint commit.
- Blockers: the linked Vercel `gala-cre` project reports no environment variables. Careers, 1031, and general Contact delivery therefore fails safely and must not be described as operational until approved Resend sender/recipient values, Beth-and-Gaurang dual routing, allowed origins, durable rate limiting, and applicable privacy/disclosure inputs are privately configured and verified. The previously recorded 202 North Main advisor gap and unresolved 10414/10416 structure remain safely omitted. These do not block content deployment but do block form activation and unsupported claims.
- Exact next action: After explicit user authorization, fast-forward `main` to this verified branch, push, deploy the linked Vercel project, and run the documented production smoke tests without submitting forms; keep form delivery disabled until its private configuration is approved.

### 2026-09-22 — September client revision production release

- Starting branch / HEAD: `codex/september-client-updates` at `931e87e762d28396c44ed8a3eb0f5031a88cf86d`, clean; `main` and `origin/main` both at `e98d8d43667778d3a3a673e625c43a991c2225b2` with no divergence from the six-commit release branch.
- Source of truth checked: the explicit merge/push/deploy request; `AGENTS.md`; `SEPTEMBER-CLIENT-REVISION-PLAN.md`; `PROPERTY-DATA-REGISTER.md`; `SEPTEMBER-RELEASE-HANDOFF.md`; Git status, branch relationship, fetched remote history, and certified tests; the linked Vercel project and production alias; and the live production routes.
- Phase / checkpoint: promote the certified September client revision to `main`, GitHub, and Vercel, then certify the live site without submitting forms.
- Gate: the approved revision is live at the production alias, GitHub and local `main` contain the same verified history, the repository is clean, and every affected production journey matches the local production build.
- Implementation: reran the dedicated local release suite; fast-forwarded `main` to the certified release branch; pushed `main` to GitHub; deployed the linked Vercel project; confirmed the production deployment reached Ready and owns `https://gala-cre.vercel.app`; exercised the production route matrix and client-specific acceptance suite; and hardened two smoke assertions so lazy portrait loading and responsive video reflow are measured after production-network layout settles rather than at an unstable intermediate frame. No public application behavior changed after certification.
- Files / routes changed: `e2e/public-route-matrix.spec.ts`, `e2e/september-client-release.spec.ts`, `SEPTEMBER-RELEASE-HANDOFF.md`, `SEPTEMBER-CLIENT-REVISION-PLAN.md`, and this ledger. Production verification covers `/`, `/team`, `/contact`, `/properties`, all three advisor-filtered catalogs, all eight property routes and inquiry contexts, all three video listings, and all service/capability routes.
- Verification: local release acceptance passed 3/3 against the production build. The live Vercel alias returned HTTP 200; all three production portraits loaded at their full intrinsic dimensions with no failed asset responses; all ten production browser scenarios passed across the complete 31-route desktop/tablet/mobile matrix plus catalog, video, inquiry, keyboard/focus, reduced-motion, Team, portfolio, relationship, privacy, console, image, and overflow assertions. No form was submitted.
- Intended commit: `test: harden production release smoke tests` after scoped diff review.
- Remaining working tree: expected clean after commit.
...
### 2026-09-22 — Post-release Company and Team consolidation

- Starting branch / HEAD: `main` at `cd2534b3169a78ed5c1b5283154f73600b9c2a16`, clean and synchronized with `origin/main`.
- Source of truth checked: the user's approved consolidation plan; `AGENTS.md`; `SEPTEMBER-CLIENT-REVISION-PLAN.md`; `PROPERTY-DATA-REGISTER.md`; the latest checkpoint in this ledger; Git status and recent history; current Company, Team, shared roster, header, footer, anchor manager, route configuration, responsive styles, tests, and live local routes.
- Phase / checkpoint: consolidate the Company and Team navigation/content journeys while preserving the complete approved roster and every existing agent relationship.
- Gate: Company is the single top-level firm-and-people destination; `/company` contains the complete shared Team roster after “How We Work”; desktop and mobile headers omit Team; footer/internal links reach `/company#team`; legacy `/team` redirects safely; profile contacts and portfolios remain correct.
- Implementation: extracted the roster into reusable `TeamRoster`; embedded it in Company with one H1 and an H2/H3 section hierarchy; removed Team from both header menus; updated Company and footer links to the Team anchor; converted the old Team page into a compatibility redirect; retained the shared-data biographies, portraits, licenses, direct contacts, and listing/transaction actions; added lazy portrait loading; and retained global fixed-header anchor clearance without a duplicate section offset.
- Files / routes changed: `src/components/team/TeamRoster.tsx`, `src/pages/Company.tsx`, `src/pages/Team.tsx`, `src/components/site/SiteHeader.tsx`, `src/components/site/SiteFooter.tsx`, `src/styles/gala.css`, `src/test/gala-pages.test.tsx`, `e2e/september-client-release.spec.ts`, `LAUNCH-READINESS.md`, `SEPTEMBER-CLIENT-REVISION-PLAN.md`, and this ledger; `/company`, `/company#team`, and legacy `/team`.
- Verification: focused Company/Team and anchor tests passed 39/39; full Vitest passed 116/116 across 18 files; production build passed with the existing browserslist-age and bundle-size notices; lint passed with 0 errors and the same 7 existing shared-UI Fast Refresh warnings; full Playwright passed 11/11. Live local Chrome QA confirmed six desktop navigation destinations with Company current, the integrated roster and all actions, the `/team` → `/company#team` redirect, and clean anchor positioning beneath the fixed header. Automated desktop, tablet, and mobile checks reported no horizontal overflow or route errors. No form was submitted.
- Intended commit: `refactor: consolidate team into company` after scoped diff review.
- Remaining working tree: expected clean after the checkpoint commit.
...
### 2026-09-22 — Company refinement Phase 2 narrative rebuild

- Starting branch / HEAD: `main` at `f23e887bc4d0e72702bf37feedc0c7aaf00fca7c`, clean and two local commits ahead of `origin/main`.
- Source of truth checked: the user's approved Phase 2 scope; `AGENTS.md`; `SEPTEMBER-CLIENT-REVISION-PLAN.md`; `PROPERTY-DATA-REGISTER.md`; `COMPANY-STORY-REGISTER.md`; the latest checkpoint in this ledger; Git status and recent history; current Company and shared Team implementation; approved Team portraits; responsive styles; tests; and the live local `/company` and `/team` routes.
- Phase / checkpoint: Company refinement Phase 2 — rebuild the non-Team Company narrative as a shorter genuine-image hero, developer-informed rationale, and concise connected operating model while preserving the verified roster.
...
- Implementation: replaced the generic black hero with a split hero using the three approved real Team portraits and **The Developer's Brokerage** positioning; replaced the abstract Purpose copy with an evidence-safe explanation of the interdependence among transaction, site, capital, operations, and execution decisions; replaced the icon principles grid with an ordered Evaluate, Position, Structure, Execute model; removed the redundant standalone Triangle section; updated metadata; and preserved the Team roster and closing CTA.
- Files / routes changed: `src/pages/Company.tsx`, `src/styles/gala.css`, `src/test/gala-pages.test.tsx`, `e2e/september-client-release.spec.ts`, `SEPTEMBER-CLIENT-REVISION-PLAN.md`, and this ledger; `/company` and legacy `/team`.
- Verification: focused Company tests passed 36/36; full Vitest passed 116/116 across 18 files; production build passed with the existing browserslist-age and bundle-size notices; lint passed with 0 errors and the same 7 existing shared-UI Fast Refresh warnings; full Playwright passed 11/11. Live local Chrome QA confirmed the real-photo desktop hero, Why Gala hierarchy, four-step operating model, and clean transition into Team. Automated 1440×900 and 390×844 acceptance confirmed the narrative, three hero images, four operating steps, legacy redirect, all Team actions, zero page/console errors, and no horizontal overflow. No form was submitted.
- Intended commit: `refactor: sharpen company story and operating model` after scoped diff review.
- Remaining working tree: expected clean after the checkpoint commit.
- Blockers: the confirmed origin sentence, formation/brand dates, approved corporate-relationship language, broader market boundary, basis for “two decades,” and additional general-use company/site photography remain client inputs and are omitted. The private production form configuration remains a separate previously recorded blocker.
- Exact next action: present the locally rebuilt `/company` route for user review, then implement only the next user-approved Company proof, market-context, or conversion checkpoint without disturbing the verified Team system or inventing unresolved facts.

### 2026-09-22 — Company refinement Phase 3 completion and QA

- Starting branch / HEAD: `main` at `0c809239a02ac4422cf4d8331d7a4ff539798bb8`, clean and three local commits ahead of `origin/main`.
- Source of truth checked: the user's approved Phase 3 scope and final page sequence; `AGENTS.md`; `SEPTEMBER-CLIENT-REVISION-PLAN.md`; `PROPERTY-DATA-REGISTER.md`; `COMPANY-STORY-REGISTER.md`; the Phase 2 checkpoint; Git status and recent history; current Company and Team implementation; approved imagery; metadata and internal links; responsive styles; automated tests; and the live local Company/Team routes.
- Phase / checkpoint: Company refinement Phase 3 — complete the Company journey with a compact evidence layer, deliberate Team transition, verified Triangle presence, two-action close, and desktop/tablet/mobile QA.
...
- Implementation: added a three-part non-metric proof layer linked to the public Properties and News systems; revised the Team introduction so the connected model resolves into clear individual responsibility; equalized Team-card presentation without changing records or actions; added a concise, text-led Cary/Research Triangle presence band because no general-use geographic image is approved; reduced the final CTA to **Let’s Connect** and **Explore Careers** with preserved source context; updated metadata; and added keyboard, image-load, anchor-position, tablet, CTA, and overflow coverage.
- Files / routes changed: `src/pages/Company.tsx`, `src/components/team/TeamRoster.tsx`, `src/styles/gala.css`, `src/test/gala-pages.test.tsx`, `e2e/september-client-release.spec.ts`, `SEPTEMBER-CLIENT-REVISION-PLAN.md`, and this ledger; `/company`, `/company#team`, and legacy `/team`.
- Verification: focused Company page tests passed 36/36; focused Company browser acceptance passed 3/3 at 1440×900, 834×1112, and 390×844; full Vitest passed 116/116 across 18 files; production build passed with the existing browserslist-age and bundle-size notices; lint passed independently with 0 errors and the same 7 existing shared-UI Fast Refresh warnings; full Playwright passed 12/12, including the 31-route desktop/tablet/mobile matrix. Live local Chrome QA confirmed the proof-to-Team transition, equal-height Team cards, compact Triangle presence, and two-action close. Company acceptance verified keyboard entry through the skip link, loaded hero images, Team anchor clearance below the fixed header, no horizontal overflow, no page/console errors, and preserved agent actions. No form was submitted.
- Intended commit: `feat: complete company journey` after scoped diff review.
- Remaining working tree: expected clean after the checkpoint commit.
- Blockers: no blocker for the completed Company journey. The origin sentence, formation/brand dates, corporate-relationship language, broader market boundary, experience basis, and additional general-use office/site/market photography remain optional client inputs and stay omitted. Private production form configuration remains a separate previously recorded blocker.
- Exact next action: present the completed local `/company` route for user review; after explicit approval, push the four verified local commits on `main`, deploy the linked Vercel project, and smoke-test `/company`, `/company#team`, and legacy `/team` in production without submitting any form.

### 2026-09-23 — Company full-bleed hero refinement

- Starting branch / HEAD: `main` at `061526c2e769f2a24a1aa70d23905565771302fa`, clean and four local commits ahead of `origin/main`.
- Source of truth checked: the user's selected `Company-hero.avif` and instruction to use it; `AGENTS.md`; `SEPTEMBER-CLIENT-REVISION-PLAN.md`; `PROPERTY-DATA-REGISTER.md`; `COMPANY-STORY-REGISTER.md`; the Phase 3 checkpoint; Git status and recent history; current Company implementation, responsive styles, assets, and Company acceptance coverage.
- Phase / checkpoint: replace only the Company introduction imagery with the selected full-bleed architectural hero while preserving the approved page sequence and all Team content.
...
- Implementation: packaged the 3840×2160, 157 KB AVIF as `src/assets/company-hero.avif`; replaced the former three-portrait grid with one eager-loading decorative hero image; added layered contrast gradients; preserved the existing headline, copy, and Services action; and created a narrow-screen crop that moves the architectural mark above the headline instead of allowing it to compete with the text. Registered the asset as conceptual brand imagery rather than documentary office or employee photography.
- Files / route changed: `src/assets/company-hero.avif`, `src/pages/Company.tsx`, `src/styles/gala.css`, `e2e/september-client-release.spec.ts`, `COMPANY-STORY-REGISTER.md`, `SEPTEMBER-CLIENT-REVISION-PLAN.md`, and this ledger; `/company` only, with legacy `/team` exercised through the existing acceptance journey.
- Verification: full Vitest passed 116/116 across 18 files; production build passed with the existing browserslist-age and bundle-size notices; lint passed with 0 errors and the same 7 existing shared-UI Fast Refresh warnings; focused Company browser acceptance passed 3/3 at 1440×900, 834×1112, and 390×844. Visual screenshots at all three widths confirmed copy contrast, intentional crops, architectural-logo visibility, and no overlap between the mobile headline and wall mark. No form was submitted.
- Intended commit: `feat: add full-bleed company hero` after scoped diff review.
- Remaining working tree: expected clean after the checkpoint commit.
