...
This repository is in a focused property-listing and service-backpage rebuild. Before changing code, read these files in order:

1. `OVERNIGHT-BUILD-PLAN.md` — authoritative phase plan, inventory, completed checkpoints, and exact next action.
2. `PROPERTY-DATA-REGISTER.md` — authoritative property facts, conflicts, publication limits, and client-input gaps.
3. Current branch, `HEAD`, working-tree status, and recent local history.
4. Current implementation, tests, and assets for the recorded next action.
...
- Preserve Team, Careers, Company, Contact, News, and the footer unless a broken link from the approved property/service work requires a correction.
